  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex justify-center items-start pt-40 px-4">
      <div className="bg-white w-full max-w-xl p-6 rounded-xl shadow-lg relative">
        ...
        <h2 className="text-2xl font-semibold mb-4">Search Mentors</h2>
        <input
          type="text"
        ...
        />
          ...
          {searchTerm.length > 0 && (
            <div className="absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-md shadow-lg mt-2 z-10 max-h-60 overflow-y-auto">
              {filteredMentors.length > 0 ? (
                filteredMentors.map((mentor) => (
                  ...
                  >
                    ...
                    <div className="text-sm text-gray-500">{mentor.skill}</div>
                  </div>
                ))
              ) : (
